...
class SubaccountHistorySection extends StatelessWidget {
  const SubaccountHistorySection({
    super.key,
      ...
      children: [
        ...
        if (entries.isEmpty)
          ...
          for (var index = 0; index < entries.length; index++) ...[
            ...
            if (index != entries.length - 1) SizedBox(height: spacing.s8),
          ],
          if (isLoadingMore) ...[SizedBox(height: spacing.s12), const Center(child: DSLoader())],
          if (canLoadMore) ...[
            SizedBox(height: spacing.s12),
            DSButton(
...
class _HistoryEntryCard extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final convertedSnapshot = factor == null ? null : entry.snapshotAmount * factor;
    ...
    final deltaColor = diff == null
        ...
        : (diff.compareTo(Decimal.zero) >= 0 ? context.dsColors.success : context.dsColors.danger);
    ...
    final snapshotAsset = code == l10n.notAvailable
        ...
        : context.dsFormatters.formatMoney(entry.snapshotAmount, code, maximumFractionDigits: 8);

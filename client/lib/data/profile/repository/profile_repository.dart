...
class ProfileRepository implements IProfileRepository {
    ...
    try {
      ...
      if (existing == null) {
        final created = await _dataSource.upsertProfile(
          ProfileMapper.toDto(ProfileEntity(userId: userId, baseCurrency: 'USD', plan: 'free')),
        );
        final entity = ProfileMapper.toEntity(created);
        logger.i('ProfileRepository.ensureProfile created');
        return Success(
          ProfileBootstrapEntity(profile: entity, isNew: true, wasBaseCurrencyDefaulted: true),
        );
      }
      ...
      return const FailureResult(Failure(code: 'unknown', message: 'Unable to load profile'));
    }
  }
  ...
  Future<Result<ProfileEntity>> updateBaseCurrency(String userId, String baseCurrency) async {
    try {
      ...
      return const FailureResult(Failure(code: 'unknown', message: 'Unable to update profile'));
    }
  }
}
